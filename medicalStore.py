...
login=sqlite.connect("/Users/mohit/Desktop/pharmacy-management-master/admin.db")
l=login.cursor()

c=sqlite.connect("/Users/mohit/Desktop/pharmacy-management-master/medicine.db")
cur=c.cursor()
...
def open_win(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    Label(apt, text="APEX MEDICAL CHEMIST AND DRUGGIST").grid(row=0,column=0,columnspan=3)
...
def ren():
    ...
    for i in cur:
        ...
        lb2.insert(cx,str(i[3])+'  '+str(i[6])+'  '+str(i[4]))
    c.commit()
...
def sel_del(e):
    ...
    for i in cur:
        if i[0]==sl2:
            Label(d,text=str(i[0])+'. '+str(i[1]),bg='white').grid(row=0,column=1)
    c.commit()
...
def ref(): # creates a multi-listbox manually to show the whole database
    ...
    for i in cur:
        ...
        lb4.insert(cx,i[4])
        lb5.insert(cx,i[5])
        lb6.insert(cx,str(i[6])+'    '+str(i[7])+'    '+str(i[8]))
    c.commit()
...
def billing(): # to create bills for customer-------------------------------------------------------------BILLING system
    ...
    Button(st,width=15,text='Add to cart', command=make_bill).grid(row=7,column=6)
...
def make_bill(): # makes bill
    ...
    messagebox.showinfo("Notice", "Added to your cart")
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    if flag=='apt':
        apt.destroy()
    root=Tk()
    root.title('APEX MEDICAL SOFTWARE SOLUTIONS')
    root.geometry("650x230")

    background_image = PhotoImage("/Users/mohit/Desktop/pharmacy-management-master/medical.png")
    background_label = Label(root, image=background_image)
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
    background_label.image = background_image


    load = Image.open("/Users/mohit/Desktop/pharmacy-management-master/medical.jpeg")
    ...
    #logo = PhotoImage(file="/Users/mohit/Desktop/pharmacy-management-master/medical.png")

    #w1 = Label(root, image=logo).pack(side='top', fill='both', expand='yes')

    Label(root,text='APEX MEDICAL').grid(row=0,column=0,columnspan=5)
    Label(root,text="CHEMIST AND DRUGIST").grid(row=1,column=0,columnspan=5)
